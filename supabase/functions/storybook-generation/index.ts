...
Deno.serve(async (req) => {
  ...
  if (!seed) {
    return new Response(JSON.stringify({ error: "No seed provided" }), { status: 400 })
  }

  // Parses [{"animal" : "cat"}, {"food": "chicken"}] into "animal: cat, food: chicken"
  const parsedAttributes = Array.isArray(seed) 
    ? seed.map(obj => 
        Object.entries(obj)
          .map(([k, v]) => `${k}: ${v}`)
          .join(', ')
  ...
  // Add instruction context so DALL-E/GPT understands the intent
  const prompt = `Generate a creative interpretation based on these keywords: ${parsedAttributes}`
  // --- END PARSING LOGIC ---
